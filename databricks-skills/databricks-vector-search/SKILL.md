...
aidevkit vector-search index list --endpoint-name my-endpoint
aidevkit vector-search index delete --name catalog.schema.my_index

# Data operations
aidevkit vector-search data sync --index-name catalog.schema.my_index
aidevkit vector-search data scan --index-name catalog.schema.my_index --num-results 100
aidevkit vector-search data upsert --index-name catalog.schema.my_index --data '[{"id":"1",...}]'
aidevkit vector-search data delete --index-name catalog.schema.my_index --primary-keys '["1","2"]'

# Query
aidevkit vector-search query --index-name catalog.schema.my_index \
    --query-text "machine learning" --columns id,content --num-results 5
```
...
- **Management vs runtime** — CLI commands handle lifecycle management; for agent tool-calling at runtime, use `VectorSearchRetrieverTool`
