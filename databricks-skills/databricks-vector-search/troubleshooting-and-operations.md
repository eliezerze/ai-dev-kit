...
Use `aidevkit vector-search endpoint get --name <endpoint>` (CLI) or `w.vector_search_endpoints.get_endpoint()` (SDK) to check endpoint health.
...
Use `aidevkit vector-search index get --name <index>` (CLI) or `w.vector_search_indexes.get_index()` (SDK) to check index health.
...
|---------------|----------|------|----------|
| **TRIGGERED** | Manual sync via `aidevkit vector-search data sync` | Lower — runs only when triggered | Batch updates, periodic refreshes, cost-sensitive workloads |
...
|-------|-------------|----------|
| **Index stuck in NOT_READY** | Sync pipeline failed or source table issue | Check `message` field via `aidevkit vector-search index get`. Inspect the DLT pipeline using `pipeline_id`. |
...
| **Sync not running (TRIGGERED)** | Sync not triggered after source update | Run `aidevkit vector-search data sync` or `w.vector_search_indexes.sync_index()` after updating source data. |
| **Endpoint NOT_FOUND** | Endpoint name typo or deleted | List all endpoints with `aidevkit vector-search endpoint list` to verify available endpoints. |
