"""Agent Bricks API - High-level Knowledge Assistant and Supervisor Agent operations.

This module provides action-based wrappers around low-level Agent Bricks functions.
Used by both MCP server and CLI.

Tools:
- manage_ka: create_or_update, get, find_by_name, delete Knowledge Assistants
- manage_mas: create_or_update, get, find_by_name, delete Supervisor Agents
"""

from typing import Any, Dict, List, Optional

from .manager import AgentBricksManager, get_tile_example_queue
...
def manage_ka(
    ...
    with_description_footer: Optional[callable] = None,
) -> Dict[str, Any]:
    ...
    - create_or_update: Create or update a KA. Requires name, volume_path.
      ...
      description: What this KA does (shown to users).
      instructions: How KA should answer queries.
      add_examples_from_volume: Scan volume for JSON example files with question/guideline pairs.
      Returns: {tile_id, operation, endpoint_status, examples_queued}.
    - get: Get KA details. Requires tile_id.
      Returns: {tile_id, name, description, endpoint_status, knowledge_sources, examples_count}.
    - find_by_name: Find KA by name. Requires name.
      Returns: {found, tile_id, endpoint_name, endpoint_status}.
    - delete: Delete KA. Requires tile_id.
      Returns: {success, tile_id}.

    Args:
        action: The action to perform.
        name: KA name (for create_or_update, find_by_name).
        volume_path: UC Volume path with documents (for create_or_update).
        description: KA description.
        instructions: KA instructions.
        tile_id: KA tile ID (for get, delete).
        add_examples_from_volume: Scan volume for examples (default True).
        on_resource_created: Callback for resource tracking.
        with_description_footer: Callback to add footer to description.
    ...
    volume_path = _none_if_empty(volume_path)
    tile_id = _none_if_empty(tile_id)
...
def manage_mas(
    ...
    Actions:
    - create_or_update: Create or update a MAS. Requires name, agents.
      agents: List of agent configs, each with:
        - name: Agent name
        - description: Critical for routing (tells supervisor when to use this agent)
      ...
      Returns: {tile_id, operation, endpoint_status, agents_count}.
    - get: Get MAS details. Requires tile_id.
      Returns: {tile_id, name, description, endpoint_status, agents, examples_count}.
    - find_by_name: Find MAS by name. Requires name.
      Returns: {found, tile_id, endpoint_status, agents_count}.
    - delete: Delete MAS. Requires tile_id.
      Returns: {success, tile_id}.

    Args:
        action: The action to perform.
        name: MAS name (for create_or_update, find_by_name).
        agents: List of agent configurations (for create_or_update).
        description: MAS description.
        instructions: Routing instructions.
        tile_id: MAS tile ID (for get, delete).
...
# ============================================================================
# Internal action handlers for KA
# ============================================================================
...
def _ka_create_or_update(
    ...
    # Apply description footer if callback provided
    if with_description_footer and description:
    ...
    if on_resource_created and response_tile_id:
        try:
            on_resource_created(
...
# ============================================================================
# Internal action handlers for MAS
# ============================================================================
...
def _mas_create_or_update(
    ...
    # Track resource on successful create/update
    if on_resource_created and response.get("tile_id"):
        try:
            on_resource_created(
                resource_type="multi_agent_supervisor",
                name=response.get("name", name),
                resource_id=response["tile_id"],
            )
        except Exception:
            pass
