"""Apps workflows - High-level Databricks Apps operations.

This module contains the business logic for app operations, used by both
...
from ..identity import with_description_footer

logger = logging.getLogger(__name__)

# Resource tracking callbacks - set by MCP server
_track_resource: Optional[Callable] = None
_remove_resource: Optional[Callable] = None


def set_resource_tracking(
    track_fn: Optional[Callable] = None,
    remove_fn: Optional[Callable] = None,
) -> None:
    """Set resource tracking callbacks (called by MCP server)."""
    global _track_resource, _remove_resource
    _track_resource = track_fn
    _remove_resource = remove_fn
...
def manage_app(
    ...
    Args:
        ...
        name: App name (required for most actions).
        source_code_path: Path to deploy from (for create_or_update).
        description: App description.
        mode: Deployment mode.
        include_logs: Include logs in get response.
        deployment_id: Specific deployment ID for logs.
        name_contains: Filter for list action.

    Returns:
        Dict with app information or error.
    """
    act = action.lower()
...
def _create_or_update_app(
    ...
    if existing:
        ...
        if _track_resource and result.get("name"):
            try:
                _track_resource(
                    resource_type="app",
                    name=result["name"],
...
def _delete_app_by_name(name: str) -> Dict[str, str]:
    ...
    if _remove_resource:
        try:
            _remove_resource(resource_type="app", resource_id=name)
        except Exception:
            pass  # best-effort tracking
