"""Compute API - High-level compute operations.

This module provides action-based wrappers around low-level compute functions.
...
from .serverless import run_code_on_serverless as _run_code_on_serverless
from .manage import (
...
def execute_code(
    ...
    Args:
        ...
        cluster_id: Cluster ID for cluster mode.
        context_id: Reuse existing context (cluster mode).
        language: "python", "scala", "sql", "r".
        timeout: Execution timeout in seconds.
        destroy_context_on_completion: Destroy context after execution.
        workspace_path: Save as notebook in workspace (omit for ephemeral).
        run_name: Name for serverless run.
        job_extra_params: Extra job params (serverless only).

    Returns:
        {success, output, error, cluster_id, context_id} or {run_id, run_url}.
    """
...
def manage_cluster(
    ...
    - start: Requires cluster_id.
    - terminate: Reversible stop. Requires cluster_id.
    - get: Returns cluster details. Requires cluster_id.
    - delete: PERMANENT. Requires cluster_id.

    Returns:
        {cluster_id, cluster_name, state, message}.
    """
...
def manage_sql_warehouse(
    ...
    - delete: PERMANENT. Requires warehouse_id.

    Returns:
        {warehouse_id, name, state, message}.
    """
    action = action.lower().strip()
...
def list_compute(
    ...
    Args:
        ...
        auto_select: Return best running cluster.

    Returns:
        Dict with requested compute information.
    """
