"""Lakebase API - High-level Lakebase operations.

This module provides action-based wrappers around low-level Lakebase functions.
Used by both MCP server and CLI.

Supports both Provisioned and Autoscale Lakebase databases.

Tools:
- manage_lakebase_database: create_or_update, get, list, delete
- manage_lakebase_branch: create_or_update, delete (Autoscale only)
- manage_lakebase_sync: create_or_update, delete (Provisioned only)
...
# Autoscale core functions - imported dynamically to avoid import errors if module not available
try:
...
def manage_lakebase_database(
    ...
    Args:
        ...
        capacity: Capacity units for provisioned (CU_1, CU_2, CU_4, CU_8).
        stopped: Start in stopped state.
        display_name: Display name for autoscale.
        pg_version: PostgreSQL version for autoscale.
        force: Force delete with children.
...
def manage_lakebase_branch(
    ...
    Args:
        action: The action to perform.
        project_name: Autoscale project name.
        branch_id: Branch identifier.
        source_branch: Source branch to fork from.
        ttl_seconds: Time-to-live in seconds.
        no_expiry: Disable TTL expiry.
        is_protected: Protect branch from deletion.
        ...
        scale_to_zero_seconds: Idle timeout before scaling to zero.
        name: Full branch name for delete.

    Returns:
        Dict with action-specific results.
    """
    if not AUTOSCALE_AVAILABLE:
        return {"error": "Lakebase Autoscale module not available"}

    act = action.lower()
    project_name = _none_if_empty(project_name)
    branch_id = _none_if_empty(branch_id)
    name = _none_if_empty(name)
...
def manage_lakebase_sync(
    ...
    Args:
        ...
        source_table_name: Source Delta table name.
        target_table_name: Target PostgreSQL table name.
        catalog_name: Unity Catalog name.
        database_name: PostgreSQL database name.
        primary_key_columns: Primary key columns for incremental sync.
        scheduling_policy: Sync scheduling policy.
    ...
    """
    act = action.lower()
    instance_name = _none_if_empty(instance_name)
    source_table_name = _none_if_empty(source_table_name)
    target_table_name = _none_if_empty(target_table_name)
    catalog_name = _none_if_empty(catalog_name)
    table_name = _none_if_empty(table_name)
...
def generate_lakebase_credential(
    ...
    Args:
        instance_names: List of provisioned instance names.
        endpoint: Autoscale endpoint name.
...
def _create_or_update_database(
    ...
    on_resource_created: Optional[callable],
) -> Dict[str, Any]:
