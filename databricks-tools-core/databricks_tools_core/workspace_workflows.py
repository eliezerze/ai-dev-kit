"""Workspace workflows - High-level workspace management operations.

This module contains the business logic for workspace connection management,
used by both the MCP server and CLI.

Tools:
- manage_workspace: status, list, switch, login
"""
...
def manage_workspace(
    ...
    Actions:
    - status: Get info about currently connected workspace.
      Returns: {host, profile, username}.
    - list: Show all profiles from ~/.databrickscfg.
      Returns: {profiles: [{profile, host, active}, ...]}.
    - switch: Change to an existing profile or host.
      Returns: {host, profile, username, message} or error.
    - login: Run OAuth via the Databricks CLI then switch.
      Returns: {host, profile, username, message} or error.

    Args:
        action: The action to perform (status, list, switch, login).
        profile: Profile name from ~/.databrickscfg (for switch).
        host: Workspace URL (for switch or login).

    Returns:
        Dict with workspace information or error.
    """
