...
Usage:
    ...
    python tests/integration/run_tests.py --report     # Show report (running or completed)
"""
...
def print_summary(results: list[TestResult], total_duration: float, output_dir: Path, run_timestamp: str = None):
    ...
    completed_count = sum(1 for r in results if r.status not in ("running", "unknown"))

    # Overall status
    if running_count > 0:
        status_color = Colors.CYAN
        status_text = f"TESTS RUNNING ({completed_count}/{len(results)} folders complete)"
    ...
    if running_count > 0:
        print(f"    {Colors.CYAN}Running:{Colors.RESET}      {running_count} folders")
    ...
    for r in sorted(results, key=lambda x: (x.status == "running", x.success, -x.failed)):
        if r.status == "running":
            status = f"{Colors.CYAN}RUNNING{Colors.RESET}"
...
Examples:
  ...
  python tests/integration/run_tests.py --all        # Run all tests including slow
  python tests/integration/run_tests.py --report     # Show report (works for running or completed)
  python tests/integration/run_tests.py -j 4         # Run with 4 parallel workers
    ...
    parser.add_argument(
        ...
        const="latest",
        metavar="TIMESTAMP",
        help="Show report from latest run (or specify timestamp). Works for running or completed tests.",
    )
    parser.add_argument(
        "--cleanup-results",
        action="store_true",
